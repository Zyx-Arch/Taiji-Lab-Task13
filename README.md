...
- 学习率：$1\times10^{-3}$
- 训练轮数：1000
- 损失函数：$\mathcal{L}=\mathcal{L}_{\text{PDE}}+\mathcal{L}_{\text{BC}}$  
  $$
  \mathcal{L}_{\text{PDE}}=\frac{1}{N_r}\sum_{i=1}^{N_r}\left(u_{xx}(x_i)+f(x_i)\right)^2,\quad
  \mathcal{L}_{\text{BC}}=\big(u(0)-0\big)^2+\big(u(1)-0\big)^2
  $$
...
1. **频谱偏差**：全连接网络在训练初期优先学习低频成分，高频成分收敛极慢。
2. **激活函数平滑性**：Tanh 等函数导数有限，难以表示剧烈振荡的函数。
3. **损失函数特性**：PDE 残差对高频误差的敏感度较低，且梯度更新时高频分量贡献小。
